...
namespace br.users.application.test.v0.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll([FromQuery] FilterUsersRequestModel requestModel, IOptions<ApiBehaviorOptions> apiBehaviorOptions)
        {
            try
            {
                var resultAsync = await _userService.GetItemsUserList(requestModel.FilterName, requestModel.FilterEmail, requestModel.FilterImg, requestModel.FilterRecentUsers, requestModel.FilterGender);
